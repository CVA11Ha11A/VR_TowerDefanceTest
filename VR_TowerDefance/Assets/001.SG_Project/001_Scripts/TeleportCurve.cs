...
public class TeleportCurve : MonoBehaviour
{
    ...
    //! 라인 렌더러를 이용해 점을 만들고 선을 그리는 함수
    private void MakeLines()
    {
        // 리스트에 담긴 위치 정보들을 비워준다.
        lines.RemoveRange(0, lines.Count);
        // 선이 진행될 방향을 정한다.
        Vector3 direction = ARAVRInput.LHandDirection * curveLeght;
    ...
    {
        // 앞 점 lastPos에서 다음 점 position으로 향하는 백터 계산하는 로직
        ...
        if(Physics.Raycast(ray,out hitInfo, rayDirection.magnitude))
        {
            ...
            // Terrain 레이어와 충돌했을 경우에만 텔레포트UI가 표시되도록 한다.
